...
Our model shows how the beneficial mutation supply--the product of population size and beneficial mutation rate--determine the evolutionary dynamics, such that with a lower mutation supply a larger fraction of the evolved population may descend from aneuploid cells. However, with high mutation supply, beneficial mutations are generated before fixation of aneuploidy, and can outcompete aneuploidy due to its inherent fitness cost.
...
It is also associated with developmental defects and lethality in other multicellular organisms~\citep{Sheltzer2011}. % trisomy 21 and Klinefelter and Turner's syndromes
...
Indeed, in a survey of 1,011 yeast strain, aneuploidy has been detected in about 19\%\citep{Peter2018}.
...
For each of two evolved populations ($H2$ and $H4$) we sequenced the ancestral diploid population (generation 0), the aneuploid population (generation 450), and the evolved euploid population (generation 1,700 or 2,350) to estimate the mutant allele frequencies (\Cref{table:mutations_H2,table:mutations_H4}). 
...
More importantly, a high mutant allele frequency in the aneuploid population was associated with a low frequency in the evolved euploid population, and vice-versa (Spearman's correlation coefficient $\rho=-0.64$ and $-0.66$ in the two experimental repetitions; \Cref{fig:mutation_frequencies}), suggesting that mutant alleles frequent in the aneuploid populations decreased in frequency when aneuploidy was lost.
...
Dashed vertical and horizontal lines show allele frequencies 20\%. Error bars show standard error of the mean accounting for the number of reads, which may be large when the total number of reads is small.
...
Similarly, for the 48 mutant alleles with high frequency in the evolved euploid populations, the highest frequencies in the aneuploid populations were 2.7\%, 7.7\%, and 11.1\% (the rest were below 1\%). 
  ...
  %TODO OD: suggest to put more stringent cut off here as a single read can be also a result of sequencing mistake. I think cut off of 10 reads is better and i think will not change the overall message 
\end{figure}
...
The estimated beneficial mutation rate is $\mu=2.965\cdot10^{-6}\ [2.718\cdot10^{-7}-3.589\cdot10^{-6}]$. From the literature, the mutation rate per base pair is roughly $2-3\cdot10^{-10}$ (refs.~\citep{Zhu2014,Lynch2008}), but it may be higher under heat stress, as several stresses may cause hypermutation in yeast~\citep{Heidenreich2007}.
If we assume a 10-fold increase over the mutation rate reported in the literature, then the estimated beneficial mutation rate can be explained by a target size of 1,000 base pairs.
\citet{Yona2012} found at least 10 genes on chromosome III that increased heat tolerance when over-expressed. Assuming that other chromosomes also have a similar number of heat-tolerance genes (i.e., chromosome III is one of the smaller chromosomes~\citep{Gilchrist2019}), and that each gene has a target size of ten base pairs, % TODO OD: on what this assumption based ? maybe look for fitness landscape analysis of other proteins and try to based estimation on that ? 
we easily get to a target size above 1,600.
...
However, this difference may be explained by an increased aneuploidy rate during heat stress~\citep{Chen2012a}.
...
Our model fits the data well: in simulations using the MAP parameter estimates, \eumt\ fixed in 61\% of simulations by generation 1,700 and in 100\% of simulations by generation 2,350~(\Cref{fig:2n*-fixation}).
...
The probability of \eumt\ descending from \anwt\ ($F_A$) increases with the aneuploidy rate, $\delta$, and decreases with both the population size $N$ and the mutation rate, $\mu$ (\Cref{fig:FA}C,D). In some cases it can also be affected by the fitness parameters (\Cref{fig:posterior_FA_supp}).
...
\begin{figure}[p]
  ...
  \caption{
	...
	\textbf{(E)} Population mean fitness in a model without drift using MAP estimate parameters. Solid lines for mean fitness with aneuploidy ($\delta>0$), where the population reaches adaptation (mean fitness at 99.99\% of maximum value) at generation 1,802. Dashed lines for mean fitness without aneuploidy ($\delta=0$), where the population adapts much earlier, at generation 535.
     ...
     % TODO OD: I missed it in the figure - make white star more visible 
  \label{fig:FA}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% TODO OD: I have some issues understanding this section (only got it right after we talked ....) I suggest to try explain better and also add the logic (why focusing on the effect on aneuploidy on mutation rate rather then mis-segregation rate ?). 
maybe also add sentence saying you also model the assumption that aneuplody affect mis-segregation rate (not shown ) 
...
They hypothesized % TODO OD: To be accurate, we never state implicitly that we think it occur in the same lineage.  
maybe you can soften here a bit "One possible/ likely scenario ...." 
 that the euploid mutant cells evolved by heat-resistance mutations in aneuploid cells followed by reversion of trisomy due to a chromosome loss event. 
