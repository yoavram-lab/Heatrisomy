...
Acceptance is determined according to the approximate likelihood~(\cref{eq:heatstress-likelihood}), which has a maximum value of 0.875. Thus, we terminated the inference when $\epsilon \le 0.13$ after six iterations, with $n_6=982$ accepted parameter vectors and effective sample size ESS=651 (\cref{fig:convergence}). % TODO for multi-locus model we need to say something else
Running the inference algorithm with different initialization seeds and less or more simulations for approximating the likelihood produced similar posterior distributions~(\cref{fig:seeds}). %TODO for multi-locus model, write about them here also.
...
We used growth curves of \euwt\ and \anwt\ in \SI{39}{\celsius} to estimate a prior distribution for $w_{2n+1}$ ~(\cref{fig:growth-curves}-D). In lieu of a better prior, we used the same prior for $w_{2n+1^*}$ and $w_{2n^*}$. 
...
Compared to other priors we tested, this prior produced lower WAIC, better posterior prediction plots, and more stable parameter estimates, as follows. %TODO IK what do you mean by better posterior prediction and is more stable parameter estimates still rellevant?
We tried to use additional growth curves. We used growth curves of \eumt\; (\emph{refined} strain from \citet{Yona2012}) and \anwt\; in \SI{39}{\celsius} to estimate $w_{2n*}/w_{2n+1}$ ~(\cref{fig:growth-curves}-L). The same prior was used for $w_{2n*}/w_{2n+1*}$. This prior resulted in WAIC 71.91, compared to 67.87 with the above informative prior. % generated with fixed-mutation-rate-and-WAICs.ipynb TODO IK update the WAIC with 1000 reps run
We also tried to use growth curves of \anwt\; and \euwt\; in \SI{30}{\celsius} to estimate $1-c$ ~(\cref{fig:growth-curves}-H). This estimation assumes that the cost of aneuploidy is the same in \SI{39}{\celsius} and \SI{30}{\celsius}; this might be incorrect, but we only assumed this to generate a prior distribution for the fitness values. The prior for $b$ was taken the same as for $c$. This prior did not converge: no parameter sets were found with an approximate likelihood greater than zero. %TODO IK check that this model had no bugs
% TODO IK it makes sense because bla bla (to explain)
...
% TODO IK, maybe wider mutation rate should be tried on a model without aneuploidy
...
The MAP (maximum a posteriori) and 50\% HDI (highest density interval) of the parameters posterior are~(\cref{fig:posterior}): %% generated by MAP-and-hdi.ipynb
mutation rate, $\mu=2.942\times10^{-6} (2.017\times10^{-7}-4.15\times10^{-6})$,
aneuploidy rate, $\delta=1.722\times10^{-3} (1.394\times10^{-3}-2.754\times10^{-3})$,
and fitness values, $w_{2n+1}=1.022 (1.021-1.023)$,
$w_{2n+1^*}=1.025 (1.024-1.026)$,
$w_{2n^*}=1.028 (1.026-1.029)$, all relative to the fitness of \euwt, which is set to $w_{2n}=1$.

% TODO IK we are jumping between b c s and w2n* w2n+1 w2n+1*. we should report all models and results either with b c s or with w. you can compute b c s from w before computing the HDIs.
This estimated aneuploidy rate agrees with previous estimates. % TODO refs (9.7*10-5 in Zhu 2014, so it's lower). YR: we need more than one ref; also what is the units of Zho 2014?
The mutation rate corresponds to a mutation target size of $10^{4}$, assuming the mutation rate per base pair is roughly $2\cdot10^{-10}$~\citep{Zhu2014} or $3.3\cdot10^{-10}$~\citep{Lynch2008}. % TODO IK please add lynch2008 to bib file
...
The single-locus model fits the data well: in simulations using the best-fit parameters (MAP estimate) \eumt\ fixed in 61\% of simulations by generation 1,700 and in 100\% of simulations by generation 2,350~(\cref{fig:2n*-fixation}-B),
Interestingly, the genotype frequency dynamics in these simulations demonstrate that \anmt\ never reaches substantial frequency (\cref{fig:ppc-plot}). % TODO move somewhere else?
...
mutation rate, $\mu=7.979\substack{+0.156 \\ -0.076}\times10^{-9}$,
fitness of mutant, $w_{2n^*}=1.013\substack{+0 \\ -0.001}$,
The estimated mutation rate without aneuploidy is much lower compared to a model with aneuploidy, suggesting a target size of just 40, because a higher mutation rate will lead to faster fixation of \eumt\ than in the experimental observations (\cref{fig:2n*-fixation}).
Thus, even disregarding the appearance of trisomic cells in the experiment, the evidence supports the inclusion of aneuploidy in the model. % TODO IK why do you say that? spell it out please - WAIC is lower? likelihood is higher? ppc looks better? what?

% TODO IK WAIC for different tau
We also inferred model parameters under the assumption that the mutation rate in aneuploid cells, compared to euploid cells, is increased by a factor $\tau$=1, 33/32 (due to an additional chromosome), 2, 5, 10 or 100 (due to genetic instability).
...
%% generated with MAP-and-hdi.ipynb
With $\tau=10$, the estimated mutation rate was slightly lower ($\mu=1.674\times10^{-6} (2.501\times10^{-7}-1.741\times10^{-6})$). 
With $\tau=100$, the estimated mutation rate was considerably lower ($\mu=3.81\times10^{-7} (1.508\times10^{-7}-4.995\times10^{-7})$) and the aneuploidy rate had higher variance ($\delta=7.824\times10^{-4} (6.616\times10^{-4}-1.462\times10^{-3})$). 
%TODO YR to reference to WAIC table also? By this table tau=100 is the best model
We cannot rule out an increased mutation rate in aneuploid cells, but we find no evidence that it does, and unless the effect is strong, it does not seem to affect our results.
...
% TODO YR Read this
Next we tried the model with fixed mutation rate $\mu$. Hence $\mu$ is not well identifiable by the model (the inferred value has high variance with no strong correlation between it and other parameters), we tried the models where mutation rate is fixed: $\mu$=$10^{-5}$, $10^{-6}$ and $10^{-7}$. We observed similar parameters inference comparing to the model with free $\mu$ parameter. However WAIC was better for the models with fixed $\mu$ ~(\autoref{table:WAIC}). The lowest WAIC got the model with $\mu$=$10^{-5}$ - 0.6, it is lower than \emph{no aneuploidy} model - 0.87, that implies higher predictive accuracy.
...
\begin{figure}[h!]
  ...
  \end{subfigure}
   \caption{
    \textbf{Genotype fixations for single-locus models with different $\tau$.} For each model, $10,000$ simulations using MAP parameters were evaluated and a percentage of simulations with fixated genotypes  \euwt, \anwt\ and \eumt\ (plots A, B and C respectively) were measured in time. \anmt\ didn't rich fixation, therefore it doesn't appear in figures. We can see that $\tau=100$ can be distinguished using data of loss of \euwt\ (plot A) and fixation/loss of \anwt\ (plot B). It is harder to distinguish $1\leq\tau\leq10$, however the correlation exists.
  }
  \label{fig:tau-plots}
\end{figure}
...
\end{equation}
We are interested in the expected time to fixation of \eumt, and the events leading to this fixation depend on the event that occurs first. If \eumt\ appears first and avoids extinction by drift, then it will fix. If \anwt\ appears first and avoids extinction, then several other events must occur for fixation of \eumt. Thus, in this aneuploid trajectory, we need to approximate the expected waiting time to reach \anmt\ and finally \eumt.
...
A deterministic-process approach is given by the classical bi-allelic single-locus equation, which provides an approximation for the expected time for genotype $i$ to reach frequency $x$ when starting from frequency $1-x$ in a population of genotype $j$ is % TODO MP is this correct?
 % TODO MP ref for equation? Crow and Kimura?
\begin{equation}
...
% TODO MP - the following remark should be removed. Instead, wherever you use one of these assumptions in the above text, you should explicitly say so (rather then say the assumptions are implicit). Then you can discuss here how these assumptions may not be met when the parameters are so and so, and that you will point it out in the results or something like that.
...
To validate the approximation in \cref{eq:T_total}, we performed simulations of the full model (\cref{eq:mutation-aneuploidy-single,eq:drift-single,eq:selection-single})  without aneuploidy, $\delta=0$ (\cref{fig:only mut}), and with the other parameters as inferred above. The approximation provides a good fit to simulation results except when the effective population size is larger than $5\cdot 10^6$ cells. In that case, the expected number of new mutants is ~0.5, and therefore we can expect that in many generations the fixation event will start with more than a single \eumt\ cell appearing in the same generation, which is in contrast to our assumption that an adaptation event starts from a single mutant cell.
 ...
 \caption{\textbf{Expected adaptation time without aneuploidy.}
  ...
  %TODO MP add panel labels A and B
  % The error bars a quite large, can you make them smaller by running more simulations?
  % Add more x ticks for panel A (10^6, 5 10 ^6 ...)
  % maybe start x axis in panel A with lower population size? 10^4 or 10^5 ? does the approximation work well there?
 \label{fig:only mut}
 \end{figure}

%\subsection*{Inference: multi-locus model.}
...
\begin{figure}[h]
	...
	% generated with growth_curves.ipynb and deg = '30' 
	\includegraphics[width=0.95\textwidth]{../figures/evo39_fitness_30deg.pdf} 
	% generated with growth_curves_refined.ipynb and deg = '30' 
...
\caption*{
	%TODO IK caption, add diff tau models
} 
\label{table:WAIC}
\end{table}
