...
%The MAP and 50\% HDI with $\tau=100$ were: 
%% generated with MAP-and-hdi.ipynb
...
To validate the approximation in \cref{eq:T_total}, we performed simulations of the full model (\cref{eq:mutation-aneuploidy-single,eq:drift-single,eq:selection-single})  without aneuploidy, $\delta=0$ (\cref{fig:only mut}), and with the other parameters as inferred above. The approximation provides a good fit to simulation results except when the effective population size is larger than $5\cdot 10^6$ cells. In that case, the expected number of new mutants is ~0.5, and therefore we can expect that in many generations the fixation event will start with more than a single \eumt\ cell appearing in the same generation, which is in contrast to our assumption that an adaptation event starts from a single mutant cell.
 ...
 \begin{figure}[h!]
 \centering
 \includegraphics[width=.49\textwidth]{../figures/figinvpNonlymut0.pdf}
 \includegraphics[width=.49\textwidth]{../figures/figinvpmuonlymut0.pdf}
 \caption{\textbf{Expected adaptation time without aneuploidy.}
  The expected time to appearance and fixation of \eumt\ as a function of the population size, $N$ (panel A) and as a function of the mutation rate, $\mu$ (panel B). Mean and 95\% CI from simulations in blue. Analytic approximation (\cref{eq:T_total}) in orange. Here, XXXX.}
  %TODO MP add panel labels A and B
  %replace XXXX with parameter values and remove them from the figure titles.
  %y label "Expected adaptation time"
  % remove legend
  % make it a single figure with two panels, then there is no need for y label on panel B
...
\begin{figure}[h]
	...
	\includegraphics[width=0.45\textwidth]{../figures/evo39_fitness_39deg.pdf} \\
	% generated with growth_curves.ipynb and deg = '30' 
	\includegraphics[width=0.45\textwidth]{../figures/evo39_fitness_30deg.pdf} \\
	% generated with growth_curves_refined.ipynb and deg = '30' 
	\includegraphics[width=0.45\textwidth]{../figures/refined_vs_evo39_fitness_39deg.pdf}
\caption{
    \textbf{Fitness estimation from growth curves.}
    % you need to give each panel a label so that you will have A-L; have A-D on same row, then E-H same row, I-L on same row;
    % caption should explain what each panel shows.
    \textbf{(A)} Fitness estimation from growth curves of \euwt\ and \anwt\ at \SI{39}{\celsius}: $\hat w_{2n+1}/w_{2n}$=1.024 (95\% CI: 0.959 - 1.115).
    \textbf{(B)} Fitness estimation from growth curves of \euwt\ and \anwt\ at \SI{30}{\celsius}: $\hat w_{2n+1}/w_{2n}$= 0.986 (95\% CI: 0.979 - 0.994).
    \textbf{(C)} Fitness estimation from growth curves of \anwt\ and \eumt\ at \SI{39}{\celsius}: $\hat w_{2n*}/w_{2n+1}$=1.033 (95\% CI: 1.027 - 1.041).
...
\begin{subfigure}{0.325\textwidth}
  ...
  % TODO maybe added dashed vertical lines with the same colors to show where the MAP of each distribution is?
