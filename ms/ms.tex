...
\begin{equation} \label{eq:phstress-likelihood} % TODO: add ph-Stress model to results or remove it from here
...
Because of the complexity of the model, these likelihood functions are intractable. Therefore, given specific values of the model parameters $\theta$, we simulate the model in many replicates to approximate the value of the likelihood function $\likelihood(\theta)$.
...
We stop the iterations, when no more sufficient change in acceptance threshold occur, and no change in posterior.
...
% TODO convergence tests, etc. Number of replicates for different models.
