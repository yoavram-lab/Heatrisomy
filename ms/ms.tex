...
The MAP frequency dynamics (\Cref{fig:ppc-plot}A) show the four genotypes but also the frequencies of \eumt\ arose from either the \anwt\ genotype via mutation and chromosome loss events, or direct from the \euwt\ genotype via mutation.
We observe that \anmt\ never reaches substantial frequency.
But more importantly, we observe that the majority % TODO add number
of \eumt\ cells are descendant of \euwt\ cells and arose via direct mutation, despite the fact that the \anwt\ genotype reaches high frequencies in the population. % TODO add number
This finding is not unique to the MAP parameter estimate. 
We simulated genotype frequency dynamics using 100,000 parameter samples from the posterior distribution (\Cref{fig:posterior}), and computed the average frequency of \eumt\ arising from \anwt at the end of the simulation, $F_A$, over 50 replicates. 
$F_A$, was % TODO add number
and only in 681 (0.6\%) of the 100,000 posterior samples was it larger than 0.5
(see \Cref{fig:ppc-plot}B) for the full histogram). 
Overall, the frequency of \eumt\ descending from \anwt\ increases with the aneuploidy rate, $\delta$, and decreases with the mutation rate (\Cref{fig:ppc-plot}C).
The cases in which $F_A>0.5$ can be roughly divided to two clusters.
The first, with a low formation rate ratio ($\delta/\mu \approx 500$) and a small fitness difference between \eumt\ and \anwt\ ($w_{\eumt}/w_{\anwt}-1 \approx 0.0055$). In these cases, aneuploidy is not generated much faster than mutation, but this is compensated by similar fitness values that decrease the fixation probability of \eumt\ and limit the strength of clonal interference.
The second cluster, with high formation rate ratio ($\delta/\mu \approx 10^5$) and a higher fitness difference between \eumt\ and \anwt\ ($w_{\eumt}/w_{\anwt}-1 \approx 0.007$). 
In these cases, the larger fitness difference between \anwt\ and \eumt\ means that once \eumt\ appears it can more easily avoid extinction and compete with \anwt; this is compensated by much faster generation of \anwt\ and can therefore lead to high $F_A$ (note that we only use samples from the posterior, that is, we only compute $F_A$ for parameter choices that reasonably agree with the experimental results.)
...
\begin{figure}[h!]
  ...
  \begin{subfigure}{0.33\textwidth}
      \centering
      \includegraphics[width=\textwidth]{../figures/scatter_dirmut_log_star_byr.pdf}
  \end{subfigure}
  \begin{subfigure}{0.33\textwidth}
      \centering
      \includegraphics[width=\textwidth]{../figures/scatter100000_rater_fitr.pdf}
  \end{subfigure}
  \caption{
    \textbf{Posterior genotype frequency dynamics.} 
    \textbf{(A)} The posterior prediction for the frequencies of the four genotypes over time. Each of the 20 curves is the average of 10,000 simulations of the model using parameters drawn from the posterior distribution.	
     }
  \label{fig:ppc-plot}
\end{figure}
