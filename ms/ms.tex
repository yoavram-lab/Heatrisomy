...
\begin{figure}[b!]
  ...
  \begin{subfigure}[t]{0.03\textwidth} \textbf{A} \end{subfigure}
  \begin{subfigure}{0.75\textwidth}
    \centering
    \includegraphics[height=1.8in]{../figures/Fig1-A.pdf}      
  \end{subfigure}
  
  \begin{subfigure}[t]{0.03\textwidth} \textbf{B} \end{subfigure}
  \begin{subfigure}{0.75\textwidth}
	\centering
	\begin{tabular}{lllll}
  ...
  \end{subfigure}
    ...
    \begin{subfigure}[t]{0.03\textwidth} \textbf{C} \end{subfigure}
    \begin{subfigure}{0.75\textwidth}
	\centering
	\begin{tabular}{llll}
	\emph{s, b, c} & s & c & b \\
	\hline
	 \emph{Calculation} & $w_{\eumt-1}$ & $1-\dfrac{w_\anmt-w_\anwt}{w_\eumt-1}$ & $w_\anwt - 1 + c$               
	\end{tabular}
	\label{table:fitness}
  \end{subfigure}
  ...
  \caption{
    ...
    \textbf{(B)} Fitness values. $s \ge 0$ is the selection coefficient of a beneficial mutation;
$0 \le c \le 1$ is the fitness cost of aneuploidy;
and $b \ge c$ is the selection coefficient, or fitness benefit, of aneuploidy.
   \textbf{(C)} Calculations of $s,b,c$ from $w_i$ values, derived from (B) expressions.
   }
  \label{fig:models}
\end{figure}
...
The fitness values of the four genotypes are given by \Cref{fig:models}B.
...
The effect of natural selection on the frequency $f_i$ of genotype $i = 2n, 2n+1, 2n+1^*, \text{or } 2n^*$ is given by
    ...
    \end{equation}
where the fitness values $w_i$ are given in \Cref{fig:models}B and $\bar{w} = \sum_{j}{f_j w_j}$ is the population mean fitness.
...
Acceptance is determined according to the approximate likelihood~(\cref{eq:heatstress-likelihood}), which has a maximum value of 0.875. Thus, we terminated the inference when $\epsilon \le 0.13$ %TODO IK for extended prior model and probably some more we used another epsilon. 
after six iterations, with $n_6=982$ accepted parameter vectors and effective sample size ESS=651 (\Cref{fig:convergence}). Running the inference algorithm with different initialization seeds and less or more simulations for approximating the likelihood produced similar posterior distributions~(\Cref{fig:seeds}). 
...
The estimated aneuploidy rate, $\delta=1.722\cdot10^{-3}\ [1.394\cdot10^{-3}-2.754\cdot10^{-3}]$ is higher than in previous studies: for chromosome III in diploid \yeast, \citet{Zhu2014} estimates  $6.7\cdot10^{-6}$ aneuploidy gain rate, \citet{Kumaran2013} estimates $3.0-4.3\cdot10^{-5}$ aneuploidy loss rate (95\% confidence interval).
...
Thus, we can infer that the benefit and cost of trisomy was $b=92\%$ and $c=89\%$, and the benefit of the beneficial mutation was $2.8\%$~(\Cref{fig:models}C). 
