...
Surprisingly, we observe that the majority of \eumt\ cells are a product of a direct mutation in \euwt\ cells, rather than descending from \anwt\ cells ($F_A^{MAP} = 0.106$, \Cref{fig:ppc-plot}A). % TODO update numbers
...
\begin{figure}[h!]
  \begin{subfigure}{\textwidth}
      ...
      \includegraphics[width=7in,height=2.5in]{../figures/full_dynamic_MAP.pdf}      
  \end{subfigure}
  \begin{subfigure}{0.5\textwidth}
      \centering
  ...
  \caption{
    ...
    \textbf{(A)} Posterior predicted genotype frequencies over time, including the source of \eumt: $2n^*_A$ arose from $2n+1$, where as $2n^*_M$ arose directly from $2n$. Shown are 50 simulations using the MAP estimate parameters.
    \textbf{(B)} Posterior distribution of $F_A$, the expected frequency of \eumt\ cells that arose from \anwt cells, computed as the average frequency at the end of 50 simulations for 86,349 samples from the parameter posterior distribution.
