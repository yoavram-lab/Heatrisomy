...
\title{Chromosomal duplication can be an evolutionary detour on the road to adaptation}
...
Aneuploidy has been implicated in cancer formation and progression~\citep{Boveri2008, Schvartzman2010,Santaguida2015review}.
...
Experimental evidence for the role of aneuploidy in adaptive evolution was demonstrated by \citet{Yona2012}. They evolved populations of \yeast under strong heat stress.
...
The authors hypothesized that aneuploidy is a \emph{transient adaptive solution}, because it can rapidly appear and fixate in the population under stressful conditions, and can then be rapidly lost when the cost of aneuploidy outweighs its benefit---after the stress is removed, or after "refined" beneficial mutations appear and fixate.

%\paragraph{The present study.}
% TODO revise
Here, we test the hypothesis that aneuploidy is a \emph{transient adaptive solution}.
First, we analyze previously unpublished sequencing data from the original experimental populations of \citet{Yona2012} to assess if the aneuploid population and the evolved euploid population carry the same mutant alleles, as expected if the later is descended from the former.
...
These results suggest that aneuploidy can be an evolutionary detour, rather than a stepping stone, on the road to adaptation.
...
Our first test of the \emph{transient adaptive solution hypothesis} employs previously unpublished sequencing data.
For each of two evolved populations ($H2$ and $H4$) we sequenced the ancestral population (generation zero), the aneuploid population (generation 450), and the evolved euploid population (generation 1,700 or 2,350) to estimate the mutant allele frequencies (\Cref{table:mutations}). % TODO add table!!
Overall, between 100 and 173 mutant alleles were detected with at least a single read in the six population that were sampled. 
...
To further test the \emph{transient adaptive solution hypothesis}, we developed an evolutionary genetic model, fitted the model to empirical data, and used it to predict the genotype frequency dynamics, or specifically, the fraction of the evolved euploid population descended from aneuploid cells. 
...
We fitted this model to the experimental results\citep{Yona2012} -- time for fixation and for loss of aneuploidy -- using approximate Bayesian computation with sequential Monte Carol (ABC-SMC)~\citep{Sisson2009}, thereby inferring the model parameters: rates and fitness effects of aneuploidy and mutation.
...
We tested the \emph{transient adaptive solution hypothesis} by estimating $F_A$, the expected frequency of \eumt\ that arose from \anwt, computed as the average frequency of such cells at the end of simulations using the MAP estimate parameters.
Surprisingly, we observe that the majority of \eumt\ cells are a product of a direct mutation in \euwt\ cells, rather than descending from \anwt\ cells ($F_A^{MAP} = 0.106$, \Cref{fig:FA}A). 
...
The mean $F_A$ was just 0.1673 [0.0154-0.370 95\% CI] and only in 489 of 100,000 posterior samples (0.489\%) $F_A$ was  larger than 0.5, that is, \eumt\ was more likely to arise from an aneuploid cell (see Supporting Material for results when transitions to less-fit genotypes are allowed, such as \eumt\ to \anmt).  
...
The probability of \eumt\ descending from \anwt\ ($F_A$) increases with the aneuploidy rate, $\delta$, and decreases with the mutation rate, $\mu$, and in some cases can also be affected by the fitness parameters (\Cref{fig:FA}C and \Cref{fig:posterior_FA_supp}).
...
They hypothesized that these evolved euploid cells were a consequence of mutations in genes that confer heat resistance, followed by reversion of trisomy by a chromosome loss event. 
...
The model predicted that only about 10\% of the evolved euploid population descended from aneuploid cells--that is, the majority of the euploid population are not descended from aneuploid cells, but rather are direct descendants of the ancestral population (\Cref{fig:FA}).

This happens despite aneuploidy reaching a high frequency in the population (>95\%). 
Conventional wisdom might suggest that once the aneuploid genotype \anwt\ reaches high frequency, it will have a better chance at producing "refined" solutions via mutations, and its descendants will come to dominate the population: the frequency \eumtA\ (which arises from \anmt) will be higher than the frequency of \eumtM\ (which arises directly from \euwt).

So how does \eumtM\ prevail?
The supply rates of \anwt\ and \eumtM\ are $N \delta \approx 11,000$ and $N \mu \approx 19$, respectively (assuming MAP parameter estimates). 
...
However, because both genotypes are produced from \euwt.
Therefore, when the frequency of \euwt\ decreases, mainly due to the increase in the frequency of \anwt, both supply rates diminish.
At this point, the higher fitness of \eumtM\ comes into play and it starts to take over the population, which at this stage is mainly composed of \anwt.
...
Although this is a stochastic process (due to drift), our results show that the time until \eumtA\ reaches a frequency of ~0.001 is roughly 450 generation, without a lot of variation (intersection of purple lines and vertical dashed line in \Cref{fig:MAP_freq_dynamics_log}).
However, by that time, \eumtM\ is already at a much higher frequency, ~0.18, and since both mutants have the same fitness, their relative frequency remains roughly the same until the end of the experiment. 
%At the first stage, \anwt\ cells appear and reach a high frequency due to their advantage under heat.
%Accordingly, \anmt\ cells appear more frequently than \eumtM. However, the fitness advantage of a "refined" mutation that emerges in the \anmt\ genotype in hindered by the cost of aneuploidy as well as the extra waiting time needed to eliminate the extra copy of the chromosome by a chromosome loss event.
%Although such chromosome loss occurs fast relative to mutation, it still leaves a time window for \eumtM\ to arise and reach high frequencies.
%Indeed, we estimate that the advantage of \anmt\ over \anwt\ is similar to the advantage of \eumt\ over \anmt (about 0.3\%), and that the rate of chromosome loss is roughly $1-3\cdot10^{-3}$.

% F_A vs N
\paragraph{Prediction for a small populations.}
We examined the effect of the population size, $N$, on the frequency of \anwt\ descendants in the evolved population, $F_A$.
We found that $F_A$ is expected to decrease as the population size increases (\Cref{fig:FA}D), ranging from about 0.9 when the population size is 10,000, to about 0.1 when the population size is above 1,000,000 (less than the experimental population size, which was 6,425,000). 
Thus, our model provides a testable prediction: if the experiment was repeated under a lower population size (via stronger daily dilutions or with in a smaller volume), then the fraction of the population descending from aneuploid cells would likely be much higher.

% mean fitness
\paragraph{Aneuploidy increases adaptation time.}
We found that aneuploidy quickly appears and reaches high frequencies but is then outcompeted by adaptive mutant lineages. 
An additional interesting outcome is that aneuploidy increases rather than decreases the adaptation time (expected time until the population mean fitness is at 99.99\% of its maximum value, which is 1,802 and 535 generation with and without aneuploidy, respectively; \Cref{fig:FA}E). 
...
The reason is that once \anwt\ is common, selection for the mutant strain (\anmt\ or \eumt) is weaker compared to when \eumt\ competes directly with \euwt.
...
We estimate that the aneuploidy rate (i.e., number of chromosome gains per generation) is $1.7 \cdot 10^{-3}$, is higher than a previous estimate of $6.7 \cdot 10^{-6}$~\citep{Zhu2016}. % TODO explain?
In addition, we find no evidence for increased mutation rates in aneuploid cells. 
Previous empirical studies have suggested that genetic instability (e.g., elevated mutation rates) in aneuploid cells is due to stress associated with the aneuploid state~\citep{Bouchonville2009,Chen2012b,Zhu2012}.
