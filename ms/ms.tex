...
For the mutation rate, $\mu$, and aneuploidy rate, $\delta$, we used uninformative uniform priors, $\mu \sim \mathit{U}(10^{-9}\ ,\ 10^{-5})$ and $\delta \sim \mathit{U}(10^{-6} , 10^{-2})$. A wider mutation rate prior, $\mu \sim \mathit{U}(10^{-9}\ ,\ 10^{-5})$, produced similar results.
...
% TODO IK replace "roughly 300" with the exact number.
...
% TODO YR Read this
Next we tried the model with aneuploidy and a fixed mutation rate $\mu$. Hence $\mu$ is not well identifiable by the model (the inferred value has high variance with no strong correlation between it and other parameters), we tried the models where $\mu$=$10^{-5}$, $10^{-6}$ and $10^{-7}$. We saw that the inference posteriors were similar to the model with free $\mu$ parameter. However WAIC was better for the models with fixed $\mu$ ~(\autoref{table:WAIC}). Model with the $\mu$=$10^{-5}$ has the best WAIC, and better than the model without aneuploidy.
