...
\end{aligned}
\end{equation}
where $\theta$ is a parameter vector, and $\mathbb{E}[\cdot]$ and $\mathbb{V}[\cdot]$ are the expectation and variance taken over the posterior distribution, which in practice are approximated using 50,000 samples from the posterior KDE. %TODO IK WAIC calculation is still under consideration, update it here after
WAIC values are scaled as a deviance measure: lower values imply higher predictive accuracy~\citep{Kass1995}.

We also plot posterior prediction: we get maximum a posteriori (MAP) estimate of the models, and for each model we execute $10,000$ simulations and plot distribution of time to fixation of \eumt, one of key properties that define the likelihood of the model to fit the data. This visualization helps to see what model fits data better, at least for it MAP parameter vector.

Also, for similar models we plot the posterior distribution of parameters, and joint distributions of parameters and if they are similar we say that the models are interchangeable. We validate it by comparing HDI (highest density interval) of posterior parameters of the models.
...
Furthermore, we fitted models with a mutation rate fixed at $\mu$=$10^{-5}$, $10^{-6}$ and $10^{-7}$. We inferred similar parameters estimates compared to the model with a free $\mu$ parameter. % TODO IK where is this shown? how do you quantify it? TODO add like S5 plot and mean+hdi after simulations with 1000 reps will finish to run.
