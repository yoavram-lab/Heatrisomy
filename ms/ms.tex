...
\title{Aneuploidy is an evolutionary sidetrack on the path to adaptation}
...
In the heat-stress experiment of \citet{Yona2012}, four populations of \yeast evolved under \SI{39}{\celsius}. Aneuploidy fixed in all four experimental repetitions in the first 450 generations. Two of the repetitions, marked $H2$ and $H4$, carried no large-scale duplications other than a chromosome III trisomy. These two repetitions continued to evolve under the same conditions, wherein aneuploidy was eliminated by generation 1,700 and 2,350.
For each of these two populations we sequenced the ancestral population (generation zero), the aneuploid population (generation 450), and the evolved euploid population (generation 1,700 or 2,350) to estimate the mutant allele frequencies (\Cref{table:mutations}).
...
Moreover, a high mutant allele frequency in the aneuploid population implies a low frequency in the evolved euploid population, and vice-versa (Spearman's correlation coefficient $\rho=-0.64$ and $-0.66$ in the two experimental repetitions; \Cref{fig:mutation_frequencies}). 
Specifically, for the 18 mutant alleles with high frequency in the aneuploid populations (>20\%), the highest frequencies in the euploid populations were 15.4\%, 16\%, 16.3\% and 19.6\% (the rest were below 15\%).
...
Thus, mutant alleles frequent in the aneuploid populations decreased in frequency when aneuploidy was lost, and most evolved euploid cells are likely not descendent from aneuploid~cells.
...
We develop an evolutionary genetic model that that includes the effects of natural selection, genetic drift, aneuploidy, and mutation to examine the role of aneuploidy in adaptive evolution.
The model follows a population of cells characterised by both their ploidy and their genotype, fit this model to the experimental results of \citet{Yona2012} using an \emph{approximate Bayesian computation} framework~\citep{Sisson2009} to infer model parameters, including rates and fitness effects of aneuploidy and mutation, and to test hypotheses about the evolutionary process by comparing different versions of the model and predicting the genotype frequency dynamics using the estimated parameter values.
...
% quote: This scenario may thus imply that the duplication of a cer- tain chromosome may provide the evolutionary time window needed for the population to search for solutions not only on the duplicated chromosome but throughout the genome.
