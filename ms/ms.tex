...
There are two stochastic events that can occur on the \euwt\ genotype background. The first is a mutation that produces a \eumt\ genotype.
The second is a mis-segregation event that produces a \anwt\ genotype.
The rate at which these events occur is significantly different, and we assume that if one event occurs (e.g., mutation), then the other event (e.g., aneuploidy) is unlikely to occur.
The expected waiting time until one of these two genotype appears and then leads to adaptation is therefore
\begin{equation}\label{eq:T_total}
T = p_a T_a+(1-p_a) T_m \;,
\end{equation}
where $p_a$ is the probability that aneuploidy is the first event to occur, 
$T_m$ is the expected time for appearance and fixation of \eumt\ given it appears before \anwt, and $T_a$ is the expected time for appearance of \anwt, followed by a mutation to \anmt, and loss of aneuploidy to \eumt, given that \anwt\ appears before \eumt.
...
We assume that the waiting time until a new genotype occurs, $T{i,j}$, is much longer than the waiting time for its fixation, $\tau_{i,j}$. Nevertheless, these fixation times must be included in a rigorous analysis. There are two ways to compute the fixation times.
...
The approximation for the expected adaptation time (\Cref{eq:T_total}) provides a good fit to simulation results except when the effective population size is above $5\cdot 10^6$ cells (\Cref{fig:only mut}).
In that case, the expected number of new mutants per generation is roughly 0.5, and  we it is likely that the fixation event will start with more than a single \eumt\ cell appearing in the same generation, which violates our assumption that a fixation event starts from a single mutant cell (i.e., $y=1/N$ in \Cref{eq:tau_fix_x}).
...
Next, we performed simulations without mutations from $\euwt$ to $\eumt$ (\Cref{fig:no direct mut}). 
...
Here, aneuploidy is transient and fixation of \anwt\ or \anmt\ could be outpaced by the arrival of the next genotype. This would explain the discrepancy between the approximations and simulation results in \Cref{fig:no direct mut}.
