...
\Cref{fig:ppc-plot}A shows the simulated frequencies of the four genotypes (\euwt, \anwt, \anmt\ and \eumt), as well as the frequencies of \eumt\ cells that arose from either \anwt\ cells via a sequences of mutation and chromosome loss events ($2n^*_A$), or directly from \euwt\ cells via a mutation event (\eumtM).
...
\begin{figure}[h!]
  ...
  \end{subfigure}
  \caption{
    \textbf{Simulated genotype frequency dynamics.}     
...
\citet{Yona2012} found that a chromosome III trisomy was acquired by \yeast populations evolving under heat stress, only to be later replaced by euploid mutant cells. Moreover, when they initiated evolutionary experiments with a population in which all cells carry a trisomy, such a replacement occurred within 1,000 generations.

They hypothesized that these evolved euploid were a consequence of (i) mutations in genes that confer heat resistance, followed by (ii) reversion of trisomy by a chromosome loss event. 
...
However, we found that this is not the case (\Cref{fig:mutation_frequencies}): mutant allele frequencies in the aneuploid and euploid populations are negatively correlated, and common alleles in the former are rare in the later. 
...
The model predicted that only about 10\% of the evolved euploid cells descended from aneuploid cells--that is, the majority of the evolved euploid cells are not descended from aneuploid cells, but rather are direct descendants of the ancestral population. This is despite aneuploidy reaching a very high frequency in the population of >95\% (\Cref{fig:ppc-plot}).

This is not an intuitive result, as conventional evolutionary reason would be that once the aneuploid genotype reaches high frequency, it will have a better chance at producing "refined" solutions via mutations.
We therefore suggest a different explanation. 
At the first stage, \anwt\ cells appear and reach a high frequency due to their advantage under heat.
Accordingly, \anmt\ cells appear more frequently than \eumtM\ (which arises directly from \euwt). However, in most cases \eumtM\ dominates the population (and not \eumtA, which arises from \anmt), because the fitness advantage of a "refined" mutation that emerges in the \anmt\ genotype in hindered by the cost of aneuploidy as well as the extra waiting time needed to eliminate the extra copy of the chromosome by a chromosome loss event. Although such chromosome loss occurs fast relative to mutation, it still leaves a time window for \eumtM\ to arise and reach high frequencies.

% quote: This scenario may thus imply that the duplication of a certain chromosome may provide the evolutionary time window needed for the population to search for solutions not only on the duplicated chromosome but throughout the genome.

%\citet{Yona2012} have analyzed the expression levels of 14 heat-shock proteins (HSP). They found that their expression level changed ``very modestly'' in the aneuploid populations, but that most of them ``showed significant up-regulation'' in the evolved euploid population. 
...
We find no evidence for increased mutation rates in aneuploid cells. 
