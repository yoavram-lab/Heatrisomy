...
\end{aligned}
\end{equation}
where $\theta$ is a parameter vector, and $\mathbb{E}[\cdot]$ and $\mathbb{V}[\cdot]$ are the expectation and variance taken over the posterior distribution. We base the calculation on 50,000 samples from the posterior KDE.
...
For the mutation rate, $\mu$, and aneuploidy rate, $\delta$, we used uninformative uniform priors, $\mu \sim \mathit{U}(10^{-9}\ ,\ 10^{-5})$ and $\delta \sim \mathit{U}(10^{-6} , 10^{-2})$. We also tried wider mutation rate prior $\mu \sim \mathit{U}(10^{-9}\ ,\ 10^{-5})$ and got similar inference.
...
This estimated aneuploidy rate agrees with previous estimates. % TODO refs (9.7*10-5 in Zhu 2014, so it's lower).
The mutation rate corresponds to a mutation target size of $10^{4}$, assuming the mutation rate per base pair is roughly $2\cdot10^{-10}$~\citep{Zhu2014}. % 0.33 * 10^(-9) in Lynch 2008 
%We can see from the posterior distribution~(\cref{fig:posterior}) the correlation between fitness of different genotypes. 
% TODO IK say/plot something about the correlation: which fitness values are correlated? how strong is it? why does it occur?
...
mutation rate, $\mu=7.979\times10^{-9} (7.903\times10^{-9}-8.135\times10^{-9})$,
fitness of mutaion, $w_{2n^*}=1.013 (1.012-1.013)$,
...
\caption*{
	TODO caption, add diff tau models
} 
\label{table:WAIC}
\end{table}
