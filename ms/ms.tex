...
%% YR revise
The ABS-SMC algorithm employs sequential importance sampling over multiple iterations~\citep{Toni2009, Klinger2017, Syga2021}.
...
The number of particles, $n_t$, is adapted at every iteration $t$ using the adaptive population strategy.
...
$K_t(\theta' \mid \theta)$ is a multivariate normal distribution. % TODO construction of K_T from \theta_{i,t-1}
...
% TODO : Actually I assume in code that  w(2n*)>w(2n+1*)>w(2n+1)>w(2n) just for speeding up the simulations (I don't check some parameters combinations in this way). But I checked also that when I don't adding this constrain the result was the same. However I didn't add reverse direction in this case.
...
% TODO use tau=1 as the default; we also check tau>1 - 33/32, 2, 5, 10, 100.
...
\begin{figure}[b!]
  ...
  % place 82% a bit higher
  % xticks - replace 500, 1000,1500,2000,2500 with 450, 1700, 2350
    \textbf{Single-locus model fit with and without aneuploidy.} Cumulative distribution of the time to fixation of \eumt\ in 10,000 simulations using the MAP estimate with and without aneuploidy (i.e., $\delta=0$ is fixed).
     The MAP likelihood (\autoref{eq:heatstress-likelihood}) is 0.86 and 0.75 with and without aneuploidy.
	}
  \label{fig:likelihood}
\end{figure}

% TODO merge previous and next figure to one figure with 2 panels
...
The distribution of adaptation time (fixation of \eumt) in $10,000$ simulations of the single-locus model with aneuploidy (blue; MAP parameters) compared to two models without aneuploidy: a model with the same parameter values except $\delta=0$ (orange); and a model fitted to the data assuming $\delta=0$ (green).
...
\paragraph{Waiting time for appearance.}
...
%TODO inference for complex model
...
%TODO combine to one figure with the previous one and with refined
...
\end{figure}
% TODO show posteriors of different iterations (with panel labels!!)
