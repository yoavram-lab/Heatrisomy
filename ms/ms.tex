...
%TODO_YR is it ok the paragraph below?  Green bars don't relate to aneuploidy lost, but to 2n* fixation...so it should be "the adaptation time of one generation was 1700.."? Correct? 
In the experiment by \citet{Yona2012}, one population lost aneuploidy by generation 1,700 and the other by generation 2,350 (dashed lines) but not before generation 450. Thus, the blue distribution is a better fit compared to the green, and the yellow histogram has a very poor fit.
The last bin contains all the simulations with time equal or greater than 3,000.
    \textbf{(B)}
	Cumulative distribution of the time to fixation of \eumt\ in 10,000 simulations using the MAP estimate with and without aneuploidy in blue and green, respectively. %TODO_YR shouldn't we write here about green that it is like in (A) - same MAP as with aneuploidy but with delta=0
...
\begin{figure}[p]
  ...
  \end{subfigure}
  \caption{
    \textbf{Likelihood profiles.} The different curves show how the single-locus model log-likelihood changes when each of the parameter changes while the other parameters remain fixed at their MAP estimates. Dashed vertical line represents the MAP value.
  }
  % trisomy rate -> aneuploidy gain rate
  % TODO_YR: To call it aneuploidy gain rate? Because it is loss rate too. I think we called it aneuploidy rate before.
