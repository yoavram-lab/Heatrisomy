...
%adapted/expanded description from Syga et al. 2021:
In short, the algorithm employs sequential importance sampling over generations. 
In generation $t$, the algorithm draws sets of parameters $\theta_i$ from a given proposal distribution and consequently simulates data $C_i$ from the model until $n_t$ instances were accepted based on the comparison to observed data $C_{obs}$ via a distance function $D(C_i, C_{obs})$ and acceptance threshold $\epsilon_t$, such that $D(C_i, C_{objs})\leq{\epsilon_t}$. As the distance function we choose $1-\likelihood$. For the acceptance threshold the median of the distances of the accepted instances of the previous generation are applied (for the first generation the acceptance threshold is defined $0.01$). 
When $n_t$ instances are accepted new proposal distribution is constructed by multivariate normal kernel with adaptive covariance matrix. In the first generation the proposal distribution equals to the prior. 
...
We used growth curves of $2n+1$ and $2n$ in \SI{39}{\celsius} to estimate  $w_{2n+1}$. We used the same prior for $w_{2n+1^*}$ and $w_{2n^*}$, assuming that fitness values shouldn't differ too much. We also tested different priors, but this one (let's call it main prior) was better in the sense of WAIC, prediction plots, estimated parameters. %TODO provide formula of WAIC. Table? To show ppc plots also?. 
\\We tested uniform prior (let's call it uninformative), $\mathit{Uniform}(1,6)$, for all $w_{2n+1}$, $w_{2n+1^*}$, $w_{2n^*}$ and only for $w_{2n+1^*}$, $w_{2n^*}$, leaving $w_{2n+1}$ prior as before. In both scenarios the estimates by the model parameters of fitness looks too high: for the first scenario the median estimates were  $w_{2n+1}=2.767$, $w_{2n+1^*}=5.79$, $w_{2n^*}=5.827$ and for the second $w_{2n+1}=1.046$, $w_{2n+1^*}=4.642$, $w_{2n^*}=4.684$. We don't expect to have such high fitness values. Moreover the WAIC for the models with these priors are 0.57 and 1.9 respectively, while WAIC for the main prior model is 0.27. Also for the model with uninformative prior for all $w_{2n+1}$, $w_{2n+1^*}$, $w_{2n^*}$ the fixation time of 2n+1 is about 16 in average, which contradicts unpublished observations that were about 250-300.
...
%TODO it make sense because bla bla?
...
\paragraph{Model comparison.}  % TODO ILIA what did you do for model comparison/selection? WAIC
