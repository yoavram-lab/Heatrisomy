...
\begin{figure}[h]
  \centering
    \includegraphics[width=0.9\textwidth]{../figures/mutation_frequencies.pdf}      
  \caption{
...
If we assume a 10-fold increase over the mutation rate reported in the literature, then the estimated beneficial mutation rate can be explained by a genomic target size of 1,000 base pairs.
...
\begin{figure}[p]
  ...
  \caption{
	...
	Yellow star shows the MAP estimate. See also \Cref{fig:posterior_FA_supp}.
...
For each $\tau$, we computed the WAIC, a criterion for model selection (lower is better, see Methods), and found WAIC is lowest for $\tau=33/32$ and $\tau=1$ (\Cref{table:WAIC}).
