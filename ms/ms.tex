...
\begin{figure}[b!]
  ...
  \caption{
    ...
    In both panels arrows denote transitions between genotypes, with transitions rates: $\mu$ and $\tau \mu$, beneficial mutation rates in euploid and aneuploid cells; $\delta$, aneuploidy gain rate; $\delta_L$, aneuploidy loss rate.
...
% TODO IK do we assume b>c?
% IK ANS we assume that 2n+1>2n, see below, so yes.
}
\end{table}
%%%
...
% TODO IK we are missing 2n+1->2n, 2n*->2n+1*, why? to neglect them we need to assume w(2n+1)>w(2n) and w(2n*)>w(2n+1*), but I thought we don't assume that but rather we infer that. 
% IK ANS We know that w(2n+1)>w(2n) because trisomy is fixated in the experiment. We can also assume that w(2n*)>w(2n+1) by Fig4A of Yona paper (refined vs trisomic growth) and because it is fixated. 
% Actually I assume in code that  w(2n*)>w(2n+1*)>w(2n+1)>w(2n) just for speeding up the simulations (I don't check some parameters combinations in this way). But I checked also that when I don't adding this constrain the result was the same. However I dind't add reverse direction in this case.
...
% YOAV: let's talk about it, I don't necessarily agree
...
% YOAV: if our inference tells us that tau=1 is as good as tau>1, then maybe we can remove this parameter from the model presentation altogether, and simply introduce it when we discuss this result as part of the sensitivity analysis section
...
Then, with probability $\mu$ or $\tau \mu$ for euploid and aneuploid cells, % TODO no tau in multi-locus model?
...
$b \ge 0$ is the selection coefficient of aneuploidy; 
and $0 \le c \le 1$ is the fitness cost of aneuploidy.
%TODO do we assume b>c ?
}
\end{table}
%%%

\paragraph{Empirical evidence.}
We use empirical results of evolutionary experiments reported by \citet{Yona2012}.
...
\likelihood(\theta\ |\ A_{450,200}\ ,\ B_{1700}\ ,\ C_{2350, 1700} ) = \\
P^4(A_{450,200}) \cdot \Big(1 - P^4(\lnot{B_{1700}} | A_{450,200} ) - P^4(\lnot{C_{2350, 1700}}| A_{450,200}) + P^4(\lnot{B_{1700}} \land \lnot{L^*_{2350, 1700}}| A_{450,200})\Big)	\;,	
\end{multline}
...
%\begin{equation} \label{eq:phstress-likelihood} % TODO: add ph-Stress model to results or remove it from here (if remained add A_t to notations, it is At,T now only)
%\likelihood(\theta | A_{150}, L_{750} ) = \\
% P^4(B_{150}) \cdot 6 \cdot	P^2(B_{600} | A_{150}) \cdot P^2(\lnot{B_{600}} | A_{150}) \;.  
% \end{equation} 
 
For a model without aneuploidy (that is, when the aneuploidy rate is fixed at zero, $\delta=0$), denote by $M_{t}$ the fixation of \eumt\ by generation $t$, and by $Q_{t,T}$ the fixation of \eumt\ by generation $t$ conditioned on no fixation at generation $T$.
The likelihood of a model without aneuploidy with parameter vector $\theta$ for the heat-stress experiment is
...
Because our model, just like the Wright-Fisher model, is non-linear and stochastic, these likelihood functions are intractable. We overcome this problem by approximating the likelihood using multiple simulations (1,000 simulations per parameter vector for the single-locus model). Denote by $\hat{\likelihood}(X | C)$ the approximate likelihood based on data of simulations $X$ and the expected conditions $C$.
%TODO if we have more models, write how much replicates were used.
% TODO ref to place where we show 1,000 is enough

\paragraph{Parameter inference.} To infer model parameters, we use approximate Bayesian computation~\citep{Sunnaker2013} with a sequential Monte-Carlo scheme, or ABC-SMC~\citep{Sisson2009}, implemented in the \texttt{pyABC} Python 
package~\citep[\href{https://pyabc.readthedocs.io}{pyabc.readthedocs.io}]{Klinger2018}. This approach uses numerical stochastic simulations of the model to infer a posterior distribution over the model parameters. It is a method of likelihood-free, simulation-based inference~\citep{Cranmer2020}, that is, for estimating a posterior distribution when a likelihood function cannot be directly computed. It is therefore suitable to our case, in which the likelihood function can only be approximated from simulations, and cannot be directly computed. 
%a bit adapted/expanded description from Syga et al. 2021: % TODO if you adapt from it, then we should cite it.

Briefly, the ABS-SMC algorithm employs sequential importance sampling over multiple iterations. 
In iteration $t$, the algorithm draws parameter vectors $\theta_i$, also called \emph{particles}, from a given proposal distribution and simulates data $X_i$ from the model until $n_t$ particles are accepted. Acceptance is based on a comparison to observed data $X_{obs}$ using a distance function $\Delta(X_i, X_{obs})$ and acceptance threshold $\epsilon_t$, such that $\Delta(X_i, X_{obs}) \leq \epsilon_t$. %TODO cite Syga here? 
In our case $X_i$ is 1000 %TODO check if this is the number for all models
simulations of the model, and the distance function is $1-\hat{\likelihood}(X_i | X_{obs})$
% TODO something about how the weight is computed and used?
The acceptance threshold $\epsilon_t$ is chosen as the median of the distances of the accepted particles in the previous iteration and $\epsilon_0=0.01$. 
When $n_t$ particles are accepted, a new proposal distribution is constructed using a multivariate normal kernel with adaptive covariance matrix. % TODO ref
In the first iteration the prior (see below) is used as a proposal distribution. 
$n_t$ is adapted at every iteration $t$ using the \emph{adaptive population strategy}~\citep{Klinger2017}.
The algorithm terminates when no more sufficient change in acceptance threshold occur, and no change in posterior. % TODO unclear
Inference with the single-locus model terminated after four iterations with $\epsilon=0.13$, 1074 accepted parameter vectors and effective sample size (ESS) 700 (\autoref{fig:ess-single}). Running the inference algorithm with different initialization seeds and different number of simulations with produced similar outcomes (\autoref{fig:seeds}). %TODO if we have more models, write about them here also.

After producing a set of weighted particles from the the posterior distribution using the above ABC-SMC algorithm, we approximate the posterior using kernel density estimation (KDE) with Gaussian kernels. We then sample $50,000$ samples from the posterior KDE to compute the posterior mode, highest density interval (HDI), plotting the posterior, etc.
...
\begin{aligned}
\mathit{WAIC}(\theta, \vec{X}) &\ =\ 
-2\log\mathbb{E}[\mathcal{L}(\theta \mid \vec{X})]\ +\ 2\mathbb{V}[\log\mathcal{L}(\theta \mid \vec{X})]
\end{aligned}
\end{equation}
where $\theta$ is a parameter vector, $X$ are the model or experiment results (i.e., fixation of aneuploidy by generation $t$ etc.), and $\mathbb{E}[\cdot]$ and $\mathbb{V}[\cdot]$ are the expectation and variance taken over the posterior distribution $P(\theta \mid \vec{X})$.
WAIC values are scaled as a deviance measure: lower values imply higher predictive accuracy and a difference of 2 is a popular threshold for model comparison~\citep{Kass1995}.
% TODO do we need to say "and a difference of 2 is a popular threshold for model comparison."? Meanwile WAICs are so small that there's no such difference
% YOAV: let's discuss
 % TODO do we use more methods? Describe probably posterior plots comparison method, and Bayes factor if it will used.
...
We used growth curves of \euwt\ and \anwt\ in \SI{39}{\celsius} to estimate a prior distribution for $w_{2n+1}$. In lieu of a better prior, we used the same prior for $w_{2n+1^*}$ and $w_{2n^*}$. Compared to other priors we tested, this prior produced lower WAIC, better posterior prediction plots, and more stable parameter estimates. %TODO To show ppc plots also? Write paragraph about prediction plots in model comparison section.
...
%YOAV: if you have an explanation, you can add it
...
% TODO what about aneuploidy loss?
...
The MAP (maximum a posteriori) and 50\% HDI (highest density interval) of the parameters posterior are~(\autoref{fig:posterior}): % TODO I changed "mode" to "MAP", ok?
mutation rate, $\mu=3.06\substack{+1.229 \\ -2.84}\times10^{-6}$,
aneuploidy gain rate, $\delta=1.61\substack{+1.122 \\ -0.197}\times10^{-3}$,
% TODO what about aneuploidy loss?
fitness of \anwt, $w_{2n+1}=1.022\substack{+0.001 \\ -0.002}$,
fitness of \anmt, $w_{2n+1*}=1.025\substack{+0.001 \\ -0.001}$,
and fitness of \eumt, $w_{2n*}=1.028\substack{+0.001 \\ -0.002}$, all relative to the fitness of \euwt, which is set to $w_{2n}=1$.
This estimated aneuploidy gain rate agrees with previous estimates. % TODO refs
...
% TODO say/plot something about the correlation, is it interesting?). 
% YES but the sentence above is not enough.
...
%TODO mutation rate tick labels should be in 10^-6, not 10^-5, and write 10^-6 not 1e-5; also it looks like you get positive density for mutation rate = 0 which doesnt make sense
% change trisomy rate to aneuploidy gain rate; what about loss rate?
% remove the y-ticks from the diagonal, they are meaningless, right?
% fitness 2n+1 (not 2n+1 fitness)
% in some cases the MAP doesn't look correct on the plot - did you compute the MAP on the joint for all 5 parameters or on the marginal of each parameter? something is weird here.
On the diagonal, the posterior kernel density estimate for each parameter. 
Below the diagonal, joint posterior density of two parameters (dark purple and bright yellow for low and high density, respectively. Red markers and orange lines for the MAP estimates. % TODO change dashed orange to red and make it a bit bolder.
} 
  \label{fig:posterior}
\end{figure}
...
The single-locus model fits the data well: in simulations using the MAP estimate parameters \euwt\ fixed in 59\% of simulations by generation 1,700 and in 100\% of simulations by generation 2,350~(\autoref{fig:2n*-fixation}) and \anwt\ fixed in roughly 300 generations, which agrees with experimental results.
...
However, a model without aneuploidy, i.e. where the aneuploidy gain rate is fixed at zero $\delta=0$, cannot explain the data~(\autoref{fig:2n*-fixation} and \autoref{fig:likelihood}). The estimated parameter values without aneuploidy are:
...
2n* fitness - $1.011\substack{+0.001 \\ -0.001}$. 
We can see that mutation rate for this model is much lower than for the model with aneuploidy. Higher mutation rate would cause quicker fixation of \eumt\ than expected by the experiment ( \autoref{fig:2n*-fixation} blue bar). Thereby, adding aneuploidy to the model, helps to attain good fit with higher mutation rate.
