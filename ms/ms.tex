...
%adapted description from Syga et al. 2021:
In short, the algorithm employs sequential importance sampling over generations. In generation $t$, the algorithm draws sets of parameters $\theta$ from a given proposal distribution and consequently simulates data $C$ from the model until $n_t$ instances were accepted ($n_t$ "particles") based on the comparison to observed data $C_{obs}$ via a distance function $D(C, C_obs)$ and acceptance threshold $\epsilon_t$, such that $D(C, C_objs)\leq{\epsilon_t}$. As the distance function we chose $1-\likelihood$. For the acceptance threshold the median of the distances of the accepted instances of the previous generation were applied (for the first generation the acceptance threshold is defined $0.01$). 
When $n_t$ instances were accepted new proposal distribution is constructed by multivariate normal kernel with adaptive covariance matrix. In the first generation the proposal distribution equals to the prior. 
The size of $n_t$ was determined by adaptive population strategy~\citep{Klinger2017}.
