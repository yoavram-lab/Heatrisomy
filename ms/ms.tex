...
% Abstract 250 words
\begin{abstract} 
Aneuploidy is common in eukaryotes, often leading to decreased growth and fitness. However, evidence from yeast and other fungi, as well as human tumour cells, suggest that specific aneuploidies can be beneficial under stressful conditions and facilitate adaptation.
In a prominent experimental evolutionary study with yeast, populations evolving under heat stress became aneuploid, only to later revert back to euploidy after genetic mutations have accumulated. It has therefore been suggested that aneuploidy serves as a "stepping stone" on the path to adaptation.
...
Second, we develop an evolutionary model with both aneuploidy and mutation and fit it to the experimental results using Bayesian inference. We then predict the genotype frequency dynamics during the experiment, demonstrating that the majority of the evolved euploid population likely did not descend from aneuploid cells, but rather from the euploid wildtype population.
...
\begin{figure}[h]
  \centering
    \includegraphics[width=0.7\textwidth]{../figures/model_illustration.pdf}      
  \caption{
...
Our model fits the data well: \eumt\ fixed in 61\% of simulations by generation 1,700 and in 100\% of simulations by generation 2,350~(\Cref{fig:2n*-fixation}).
