...
We assume the waiting time for appearance of \eumt\ from \euwt\, of \anwt\ from \euwt\, of \anmt from \anwt and of \eumt from \anmt are geometrically distributed with rates $p_{2n^*}$, $p_{2n+1}$, $p_{2n+1^*}$ and $p_{\eumt_\delta}$, respectively, where
\begin{subequations}
\begin{align}
p_{2n, 2n^*} &= 1-\Big(1-2\mu \frac{w_{2n^*}-w_{2n}}{w_{2n}}\Big)^N \;, \\
p_{2n, 2n+1} &= 1-\Big(1-2\delta \frac{w_{2n+1}-w_{2n}}{w_{2n}}\Big)^N \; \\
p_{2n+1, 2n+1^*} &= 1-\Big(1-2\mu \frac{w_{2n+1^*}-w_{2n+1}}{w_{2n+1}}\Big)^N \; \\
p_{2n+1^*, 2n^*} &=1-\Big(1-2\delta \frac{w_{2n^*}-w_{2n+1^*}}{w_{2n+1^*}}\Big)^N \;.
\end{align}
\end{subequations}

However, we have two competing events: if one event occurs (e.g., mutation), then the other event (e.g., aneuploidy) is unlikely to occur. Thus, we need to find the minimum of these waiting times. The minimum of two geometric random variables with rates $p_{2n,2n^*}$ and $p_{2n,2n+1}$ is also a geometric random variable with rate $1-(1-p_{2n,2n^*})(1-p_{2n,2n+1})$, which is the inverse of the expected waiting time until the first event occurs.

Given a mutation occurs first, % TODO MP: I added this sentence, is that right? I guess.
the expected adaptation time via the mutation trajectory is 
\begin{equation}\label{eq:T_mut}
T_{M}=\frac{1}{1-(1-p_{2n,2n^*})(1-p_{2n,2n+1})} + T_{2n, 2n^*} \;,
\end{equation}
where $T_{i, j}$ is the waiting time for fixation of genotype $j$ in a population of genotype $i$, given it starts with a single copy and does not go to extinction by drift.
...
% TODO MP: p_{2n+1^*} is not defined, and why is there 1/p_{2n^*} here? shouldn't it have the waiting time for 2n+1* to appear on 2n+1 and then waiting time for 2n* to appear on 2n+1*? something is wrong here.
 T_A = \frac{1}{1-(1-p_{2n,2n^*})(1-p_{2n,2n+1})} + \frac{1}{p_{2n+1,2n+1^*}} + \frac{1}{p_{2n+1^*,2n^*}} + T_{2n, 2n+1} + T_{2n+1, 2n+1^*} + T_{2n+1^*, 2n^*} \;.
...
It approximates the expected time for genotype $j$ to reach frequency $x$ when starting from frequency $y$ in a population of genotype $i$,
 % TODO MP ref for equation? Crow and Kimura? YR: yes
\begin{equation} \label{eq:T_fix_x}
 T_{i, j}(x,y) \approx \frac{\log{\Big(\frac{x(1-y)}{(1-x)y} \Big)}}{\log{\Big(\frac{w_j}{w_i}\Big)}} \;,
\end{equation} 
and $T_{i,j} = T_{i,j}(1-1/N, 1/N) \approx 2\log{(N)}/\log{(w_j/w_i)}$.
A stochastic-process approach for approximating $T_{i, j}$ is given by the diffusion-equation approximation \citep[eq.~17]{Kimura1969a}. % TODO MP give details here?
...
First, to validate the approximation in \Cref{eq:T_total}, we performed simulations of the full model (\Cref{eq:mutation-aneuploidy-single,eq:drift-single,eq:selection-single}) without aneuploidy, i.e., $\delta=0$ (\Cref{fig:only mut}), and with the other parameters as inferred above.
The approximation provides a good fit to simulation results except when the effective population size is above $5\cdot 10^6$ cells. In that case, the expected number of new mutants per generation is roughly 0.5, and  we can expect that in many generations the fixation event will start with more than a single \eumt\ cell appearing in the same generation, which violates our assumption that an adaptation event starts from a single mutant cell.
