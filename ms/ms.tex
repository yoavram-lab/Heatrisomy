...
% YR: this whole section doesn't really make sense. We know there was aneuploidy. If we assume delta=0 that means that we think this aneuploidy was not relevant to the adaptation process. But then why do we take generations 1700 and 2300 to be the time for 2n* fixation? These times were not observed for fixation of a mutant but rather for elimination of aneuploidy. If we disregard aneuploidy, why do we still consider these times?
...
% TODO IK Describe posterior plots comparison method - Fig 4 & S3
...
We used growth curves of \euwt\ and \anwt\ in \SI{39}{\celsius} to estimate a prior distribution for $w_{2n+1}$ ~(\Cref{fig:growth-curves}-D). In lieu of a more suitable prior, we used the same prior for $w_{2n+1^*}$ and $w_{2n^*}$. 
...
First, we tried to use additional growth curves of \eumt\; (\emph{refined} strain from \citet{Yona2012}) and \anwt\; in \SI{39}{\celsius} to estimate $w_{2n*}/w_{2n+1}$ ~(\Cref{fig:growth-curves}L). The same prior was used for $w_{2n*}/w_{2n+1*}$. This prior resulted in WAIC 71.91, compared to 67.87 with the above informative prior. % generated with fixed-mutation-rate-and-WAICs.ipynb TODO IK update the WAIC with 1000 reps run
...
The estimated aneuploidy rate, $\delta=1.722\cdot10^{-3}\ [1.394\cdot10^{-3}-2.754\cdot10^{-3}]$, agrees with previous estimates. % TODO refs (9.7*10-5 in Zhu 2014, so it's lower). YR: we need more than one ref; also what is the units of Zho 2014?
...
Thus, we can infer that the benefit and cost of trisomy was $b=2.5\%$ and $c=0.3\%$, and the benefit of the beneficial mutation was $2.8\%$~(\Cref{table:single-locus}). 
...
The model fits the data well: in simulations using the MAP parameter estimates, \eumt\ fixed in 61\% of simulations by generation 1,700 and in 100\% of simulations by generation 2,350~(\Cref{fig:2n*-fixation}B). % TODO IK what happened to the sentence about the fixation on anwt? green lines ub fig4?
...
% commented out, see remark above when describing likelihood of delta=0.
...
%The fitness of the mutant is also much lower at $w_{2n^*}=1.013\ [1.012-1.013]$.
...
Furthermore, we fitted models with a mutation rate fixed at $\mu$=$10^{-5}$, $10^{-6}$ and $10^{-7}$. We inferred similar parameters estimates compared to the model with a free $\mu$ parameter. % TODO IK where is this shown? how do you quantify it?
...
We aim to approximate the adaptation time (i.e., time to fixation of \eumt) and analyse how it is affected by the model parameters.
There are two genotypes events that can occur on the \euwt\ genotype background. The first is a mutation that produces a \eumt\ genotype.
The second is a mis-segregation event that produces a \anwt\ genotype.
The rate at which these genotypes appear is significantly different. 
We assume that if one genotype appears (e.g., \eumt), then it will genotype rapidly fix in the population and therefore the other genotype (e.g., \anwt) is unlikely to occur. 
The expected waiting time until one of the two genotype appears and then leads to adaptation is therefore
\begin{equation}\label{eq:T_total}
T = p_a T_a+(1-p_a) T_m \;,
\end{equation}
where $p_a$ is the probability that \anwt\ is the first to appear without going to extinction, 
$T_m$ is the expected time for appearance and fixation of \eumt\ given it appears before \anwt, and $T_a$ is the expected time for appearance of \anwt, followed by a mutation to \anmt, and loss of aneuploidy to \eumt, given that \anwt\ appears before \eumt. Note that we assume the probability that \eumt\ is the first to appear without going to extinction is $1-p_a$.
...
\begin{subequations}
\begin{align}
p_{2n, 2n^*} &= 1-\Big(1 - 2 \frac{w_{2n^*}-w_{2n}}{w_{2n}} \mu \Big)^N \;, \\
p_{2n, 2n+1} &= 1-\Big(1 - 2 \frac{w_{2n+1}-w_{2n}}{w_{2n}} \delta \Big)^N \; \\
p_{2n+1, 2n+1^*} &= 1-\Big(1 - 2 \frac{w_{2n+1^*}-w_{2n+1}}{w_{2n+1}} \mu \Big)^N \; \\
p_{2n+1^*, 2n^*} &=1-\Big(1 - 2 \frac{w_{2n^*}-w_{2n+1^*}}{w_{2n+1^*}} \delta \Big)^N \;.
\end{align}
\end{subequations}
However, if the mis-segragation supply is high enough, $\delta N > 1$, then \eumt\ may appear before \anmt\ is fixed (\Cref{fig:ppc-plot}), and the latter equation is
\begin{equation}
p_{2n+1^*, 2n^*} = 1-\Big(1-2 \frac{w_{2n^*}-w_{2n+1}}{w_{2n+1}} \delta \Big)^{zN} \;,
\end{equation}
where $z \ll 1$ is the frequency of \anmt\ when \eumt\ first appears (note that here we use $w_{2n+1}$ rather than $w_{2n+1^*}$). 

We assume that if \anwt\ appears in a population of \euwt\ and does not go to extinction then \eumt\ does not appear, and vice versa. Thus, we need to find the minimum of the waiting times for appearance of these genotypes. The minimum of two geometric random variables with rates $p_{2n,2n^*}$ and $p_{2n,2n+1}$ is also a geometric random variable with rate $p_{min} = 1-(1-p_{2n,2n^*})(1-p_{2n,2n+1})$, which is the inverse of the expected waiting time until the first event occurs, $T_{min}=1/p_{min}$.
Given that one of these genotypes appeared, the probability that it was \anwt\ rather than \eumt\ is
$p_A = \frac{p_{2n,2n+1}}{p_{2n,2n^*}+p_{2n,2n+1}}$.

Given a mutation occurs first, the expected adaptation time via the mutation trajectory is 
\begin{equation}\label{eq:T_a} 
T_{M} = T_{min} + \tau_{2n, 2n^*} \;,
\end{equation}
where $\tau_{i, j}$ is the waiting time for fixation of genotype $j$ in a population of genotype $i$, given it starts with a single copy and does not go to extinction by drift.
Given that aneuploidy occurs first,
the expected adaptation time via the aneuploidy trajectory is approximated by
\begin{equation}\label{eq:T_a} 
T_a = T_{min} + T_{2n+1, 2n+1^*} + T_{2n+1^*,2n^*} + \tau_{2n, 2n+1} + \tau_{2n+1, 2n+1^*} + \tau_{2n+1^*, 2n^*} \;.
\end{equation}

We assume that the waiting time until a new genotype occurs, $T_{i,j}$, is much longer than the waiting time for its fixation, $\tau_{i,j}$. Nevertheless, these fixation times must be included in a rigorous analysis. There are two ways to compute the fixation times.
A deterministic-process approach is given by the classical bi-allelic single-locus equation.
It approximates the expected time for genotype $j$ to reach frequency $x$ when starting from frequency $y$ in a population of genotype $i$ by
 % TODO MP ref for equation? Crow and Kimura? YR: yes
\begin{equation} \label{eq:tau_fix_x}
\tau_{i, j}(x,y) \approx \frac{\log{\Big(\frac{x(1-y)}{(1-x)y} \Big)}}{\log{\Big(\frac{w_j}{w_i}\Big)}} \;,
\end{equation} 
and we use $\tau_{i,j} = \tau_{i,j}(1-1/N, 1/N) \approx 2\log{(N)}/\log{(w_j/w_i)}$, except for $\tau_{i,2n^*} = \tau_{i,2n^*}(0.95, 1/N)$ because we stop our simulations when genotype \eumt\ reaches frequency of 0.95.
A stochastic-process approach for approximating $T_{i, j}$ is given by the diffusion-equation approximation \citep[eq.~17]{Kimura1969a}. 
% TODO MP give details here

\paragraph{Validation.}
To validate our approach we performed simulations of the full model (\cref{eq:mutation-aneuploidy-single,eq:drift-single,eq:selection-single}) without aneuploidy, i.e., $\delta=0$, and with the other parameters as inferred above.
The approximation for the expected adaptation time $T$ (\cref{eq:T_total}) has a good fit to simulation results except when the effective population size is above $5\cdot 10^6$ cells (\Cref{fig:only mut}A).
In that case, the expected number of new mutants per generation is roughly 0.5, and   the fixation event is likely to start with more than a single \eumt\ cell appearing in the same generation, which violates our assumption that a fixation event starts from a single mutant cell (i.e., $y=1/N$ in \cref{eq:tau_fix_x}).
% TODO can we verify that setting y to higher value corrects the approximation?

 \begin{figure}[h!]
 \centering
 \includegraphics[width=\textwidth]{../figures/figgrid_noaneu.pdf}
 \caption{\textbf{Expected adaptation time without aneuploidy.}
  The expected time to appearance and fixation of \eumt\ as a function of the population size, $N$ (\textbf{A}) or as a function of the mutation rate, $\mu$ (\textbf{B}). Mean and 95\% CI from simulations in blue. Analytic approximation (\cref{eq:T_total}) in orange. Here, $w_\anwt=1.021,\,   w_\anmt=1.025$ and $w_\eumt=1.028$. In panel A, $\mu=9.6\cdot 10^{-8}$. In panel B, $N=6\cdot 10^6$.
  }
  %TODO MP 
  % which approximation do you use here for T_i->j ?
  % The error bars a quite large, can you make them smaller by running more simulations?
  % --> or by making the y axis in log scale!!
  % more ticks for x axis
 \label{fig:only mut}
 \end{figure}

 
Next, we performed simulations without mutations from \euwt\ to \eumt\ but with mutations from \anwt\ to \eumt\ (\Cref{fig:no direct mut}). 
% TODO MP: I comment this out - not clear what "Before" refers to, and not sure we need this sentence. what do you think?
%Before, we had the optimal scenario of waiting until the first successful mutation appears and then waiting until is reaches fixation. Our approximations did quite well there. 
Here, aneuploidy is transient and fixation of \anwt\ or \anmt\ could be outpaced by the arrival of the next genotype (\anmt\ and \eumt, respectively). This would explain the discrepancy between the approximations and simulation results in \Cref{fig:no direct mut}.
To asses this further, \Cref{fig:fixT vs waitT} provides a clear picture. At least for intermediate to high values of the respective range of the parameters, the waiting time for appearance, $T_{i,j}$ is far shorter than the time to fixation, $\tau_{i,j}$.
Indeed, if we ignore the fixation time of $\anwt$ and $\anmt$ ($T_{2n,2n+1}$ and $T_{2n+1,2n+1^*}$) in \cref{eq:T_a}, we get a better fit to the data (compare red curves and blue markers in \Cref{fig:no direct mut}).
Furthermore, setting $x=0.017$ for $T_{2n+1^*, 2n^*}$ in \cref{eq:tau_fix_x}, we get an even better fit in some cases (\Cref{fig:no direct mut}B), but not others (\Cref{fig:no direct mut}A,C). % TODO maybe remove these x=0.017 sentences and the next sentence
However, we stop simulations when genotype $\eumt$ reaches frequency of $0.95>0.017$.  


\begin{figure}[h!]
 %\centering
 \includegraphics[width=1\textwidth]{../figures/figgrid_nodirectmut_ylog_uncorr.jpg}
 \caption{\textbf{Expected adaptation time without mutations from $\euwt$ to $\eumt$.}
  The expected time to appearance and fixation of \eumt\ as a function of the population size, $N$ (\textbf{A}) and as a function of the mutation rate, $\mu$ (\textbf{B}) and as a function of the aneuploidy rate, $\delta$ ({C}). Blue error bars for mean and 95\% CI from simulations. Green line for the analytic approximation (\cref{eq:T_total}). % TODO why green? in previous figure it was orange.
  Red curve for the approximation with $T_{\euwt, \anwt}=T_{\anwt, \anmt}=0$. 
  Orange curve for the approximation with $x=0.017$ in \cref{eq:tau_fix_x} for the fixation time of $\eumt$; we found this gives a good fit in panel B.
...
\caption{
    ...
    % TODO IK caption should explain what each panel shows.
...
\begin{subfigure}{0.325\textwidth}
  ...
  % TODO IK maybe add dashed vertical lines with the same colors to show where the MAP of each distribution is?
  
  \end{figure}
 
%TODO YR read this 
%% generated with fixed-mutation-rate.ipynb 
% TODO IK run for all updated files
\begin{table}[h]
\centering
\caption{
...
\caption*{
	TODO caption
	%TODO IK caption; what is tau in models 1-5?
} 
\label{table:WAIC}
\end{table}
