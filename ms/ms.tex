...
Here, we test this hypothesis. We use Bayesian inference to fit an evolutionary model with both aneuploidy and mutation the experimental results. We then predict the genotype frequency dynamics during the experiment, demonstrating that most of the evolved euploid population likely did not descend from aneuploid cells, but rather from the euploid wild-type population.
...
Nevertheless, increasing the rate of chromosome loss (transitions from \anmt\ to \eumt) without increasing the rate of chromosome gain (transitions from \euwt\ to \anwt) increases $F_A$ (\Cref{fig:FA_rates}B), but not to the same extent as increasing the rate of chromosome gain.
